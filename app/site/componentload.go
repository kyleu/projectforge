...
var componentMetadata = map[string][]string{
	...
	"dom":          []string{"gamepad", "provides TypeScript methods for manipulating the DOM"},
	"editor":       []string{"edit", "form editor with lots of neat features"},
	"flash":        []string{"fire", "temporary notifications to the user"},
	"form":         []string{"list", "components for building forms backed by Golang data"},
	"icons":        []string{"icons", "in-document SVG references with theming support"},
	"jsx":          []string{"dna", "templates for TypeScript objects, using HTML syntax"},
	...
	"search":       []string{"search", "search framework for custom routines and generated code"},
