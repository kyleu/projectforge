...
func ThemeColor(rc *fasthttp.RequestCtx) {
	controller.Act("theme.color", rc, func(as *app.State, ps *cutil.PageState) (string, error) {
		...
		ps.Data = x
		return controller.Render(rc, as, &vtheme.Edit{Theme: x}, ps, "admin", "Themes", col)
	})
}
...
func ThemePreview(rc *fasthttp.RequestCtx) {
	controller.Act("theme.preview", rc, func(as *app.State, ps *cutil.PageState) (string, error) {
		...
		return controller.Render(rc, as, page, ps, "admin", "Themes||/admin/theme", thm.Key)
	})
}
