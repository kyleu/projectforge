...
func controllerDetail(g *golang.File, models model.Models, m *model.Model, grp *model.Column, audit bool, prefix string) *golang.Block {
	...
	ret.WE(2, `""`)
	checkGrp(ret, grp)
	ret.WF("\t\tps.SetTitleAndData(ret.TitleString()+\" (%s)\", ret)", m.Title())
	...
	if audit {
		...
		if len(pks) > 1 {
			...
			ret.WF(msg, m.Name, m.PKs().ToGoStrings("ret.", false, 160))
		}
		ret.W("\t\tif err != nil {")
	...
	if len(argKeys)+len(revArgKeys) <= 2 {
		...
		ret.WF("\t\t}, ps, %s%s, %s)", m.Breadcrumbs(), grpHistory, bcFor(m))
	}
	ret.W("\t})")
	ret.W("}")
	return ret
...
func getReverseArgs(models model.Models, m *model.Model, rrels model.Relations, ret *golang.Block) ([]string, []string) {
	...
	lo.ForEach(rrels, func(rrel *model.Relation, _ int) {
		...
		ret.WF(msg, rm.ProperPlural(), rNames, rm.Proper(), rNames, lCols.ToRefs("ret.", rCols...), rm.ProperPlural(), rNames, delSuffix)
		ret.W("\t\tif err != nil {")
		ret.WF("\t\t\treturn \"\", errors.Wrap(err, \"unable to retrieve child %s\")", rm.TitlePluralLower())
		ret.W("\t\t}")
	})
	return argKeys, argVals
}
