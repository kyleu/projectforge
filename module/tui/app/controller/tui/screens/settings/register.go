...
func Register(reg *screens.Registry) {
	...
	registerCoreScreens(reg){{{ if .HasModule "process" }}}
	registerExecScreens(reg){{{ end }}}{{{ if .HasModule "task" }}}
	registerTaskScreens(reg){{{ end }}}{{{ if .HasModule "websocket" }}}
	registerSocketScreens(reg){{{ end }}}
}
