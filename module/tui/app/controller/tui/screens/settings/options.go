...
const (
	...
	keyModules  = "settings.admin.modules"{{{ if .HasModule "process" }}}
	keyExec     = "settings.admin.exec"{{{ end }}}{{{ if .HasModule "task" }}}
	keyTask     = "settings.admin.task"{{{ end }}}
	keySitemap  = "settings.admin.sitemap"
	keyRoutes   = "settings.admin.routes"{{{ if .HasModule "websocket" }}}
	keySockets  = "settings.admin.sockets"{{{ end }}}
...
var adminOptions = []adminOption{
	...
	{Key: keyModules, Title: "Go Modules", Description: "The Go modules used by Project Forge", Route: "/admin/modules", Kind: kindScreen},{{{ if .HasModule "process" }}}
	{Key: keyExec, Title: "Managed Processes", Description: "Manage OS processes from within this app", Route: "/admin/exec", Kind: kindScreen},{{{ end }}}{{{ if .HasModule "task" }}}
	{Key: keyTask, Title: "Task Engine", Description: "See the tasks that have run, and start new runs", Route: "/admin/task", Kind: kindScreen},{{{ end }}}
	{Key: keySitemap, Title: "Sitemap", Description: "Displays available HTTP actions", Route: "/admin/sitemap", Kind: kindScreen},
	{Key: keyRoutes, Title: "HTTP Routes", Description: "Enumerates all registered HTTP routes", Route: "/admin/routes", Kind: kindScreen},{{{ if .HasModule "websocket" }}}
	{Key: keySockets, Title: "Active WebSockets", Description: "Manage the active WebSockets in this server", Route: "/admin/sockets", Kind: kindScreen},{{{ end }}}
