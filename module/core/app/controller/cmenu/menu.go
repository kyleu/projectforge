...
func MenuFor(
	...
	ret = append(ret, {{{ if .HasModule "graphql" }}}graphQLMenu(ctx, as.GraphQL), menu.Separator, {{{end}}}{{{ if .HasModule "sandbox" }}}sandbox.Menu(ctx), menu.Separator, {{{ end }}}admin{{{ if .HasModule "docbrowse" }}}, menu.Separator, DocsMenu(logger){{{ end }}}{{{ if .HasModule "mcp" }}}, MCPMenu(){{{ end }}})
